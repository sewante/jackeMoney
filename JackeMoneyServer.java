...
public class JackeMoneyServer {
    ...
    public static void main(String[] args) throws IOException{
		...
		while(true){
				...
				try{
					...
					socket = port.accept();  //establish connection
	...
	//the request command*********************************************************************************************************************
							...
							try{
								if(request.length == 3){
									...
									//check if a valid customer number is provided in the command
									//for warid number
									...
									//for mtn number
										...
										reply = "\n ERROR: Customer number, "+customerNumber+" is not supported by the system..unknown service provided \n request failed.. ";
									}
								...
								reply = " ERROR: Invalid amount submitted for request...request failed";
							}
						 }
	 ...
	 //the view requests command****************************************************************************************************************
								...
								try{
									...
									String requestResponse = " Requests: "+ "\n"+
											"Request Id  Customer Number      Amount(UGx)        Status        Service Provider" + "\n" + "\n";
	...
	//for servicing request command********************************************************************************************************
									...
									if(deposit.length == 6){
										...
										if(customerNumber.startsWith("070") || customerNumber.startsWith("075")){
											if(acceptedAmount(amount, customerNumber) == true){
												...
												updateCommissions(amount, "deposit" , customerNumber,userName);							//update the total commission for warid
											...
											if(acceptedAmount(amount, customerNumber) == true){											//is amount acceptable
												insertTransaction(amount, customerNumber, agentNumber,commission,"deposit",userName);	//record the transaction into database
												updateDepositFloatAndCash(amount, customerNumber, userName);							//update float and cash
												updateCommissions(amount, "deposit" , customerNumber,userName);							//update the total commission for mtn
												updateStatus(requestId);																//update status
				...
				/*	
						//this section of code if commented out will handle a simgle withdraw transaction
													...
													if(customerNumber.startsWith("070") || customerNumber.startsWith("075")){					//check number
														if(acceptedAmount(amount,customerNumber) == true){										//check  amount acceptable
															if(verifyWithdraw(amount, customerNumber, userName) == true){						//check if theres enough cash
																...
																updateCommissions(amount, "withdraw" , customerNumber,userName);			   //update the total commission for warid
													...
													//do the same check on the mtn number
														...
														if(acceptedAmount(amount,customerNumber) == true){										//check  amount acceptable
															if(verifyWithdraw(amount, customerNumber, userName) == true){						//check if theres enough cash
																insertTransaction(amount, customerNumber, agentNumber,withdrawCommission,"withdraw",userName);	//record the transaction into database
																updateWithdrawFloatAndCash(amount, customerNumber, userName);					//update float and cash
																updateCommissions(amount, "withdraw" , customerNumber,userName);			   //update the total commission for mtn
							   ...
							   String[] transactions = message.split(",");
								...
								if((transactions.length <= 3) && (transactions.length >= 2)){	//for two or three transaction
									...
									for(int i = 0; i < transactions.length; i++){
												...
												if(trans.length == 4){
													...
													if(customerNumber.startsWith("070") || customerNumber.startsWith("075")){
														if(acceptedAmount(amount, customerNumber) == true){							//is the amount acceptable
															...
															updateCommissions(amount, "deposit" , customerNumber,userName);			//update the total commission for warid
															System.out.println(" Warid Deposit successful..");						//status at the server
															commitReply += "\n Warid Deposit succesfully recorded..";					//inform attendant
														...
														if(acceptedAmount(amount, customerNumber) == true){												//is the amount acceptable
															...
															updateCommissions(amount, "deposit" , customerNumber,userName);			   				//update the total commission for mtn
															System.out.println(" MTN Deposit successful..");										//status at the server
															commitReply += "\n MTN Deposit succesfully recorded..";									//inform attendant
														}
														else{
															commitReply += "\n ERROR: Amount, "+String.valueOf(amount)+", is too much..Not supported by the system, deposit failed";
														}
													...
													//when number is neither warid nor mtn
														...
														commitReply += "\n ERROR: invalid customer Number,"+customerNumber+", enter a valid customer number.., deposit failed";
													}//end ensure 
												} else{
													commitReply += "\n ERROR: Invalid parameters for Deposit.., deposit failed";
												}
											}catch(NumberFormatException e){
												commitReply += "\n ERROR: Invalid amount supplied for Deposit.., deposit failed";
											}
											...
											try{
												if(trans.length == 4){
													...
													//when number is not either mtn or warid
														...
														commitReply += "\n ERROR: invalid customer Number,"+customerNumber+", enter a valid customer number..";
													}//end esure
												} else{
										...
										commitReply += "\n ERROR: Transactions are Either Deposit or withdraw only..";
										}
									}
									
									reply = commitReply;
								 } else{
									reply = "\n ERROR: Error in commit... Only a minimum of two and a maximum of three transactions are possible at a time..";
								}
						} else{
							reply = "\n ERROR: Un recogonised command: " + message+", make sure you typed the correct command...";
						}
...
//update status and service time
    public static void updateStatus(int requestId) throws SQLException{
        ...
        String newStatus = "update request set status='served',dateAndTime='"+newTime+"' where requestId="+requestId;
    ...
    public static void updateCommissions(int amount, String type, String customerNumber, String user) throws SQLException {
        ...
        if(type.equals("deposit")){
            ...
            //do same for mtn deposit
                ...
                if(mtnCommision.next()){		//is there mtn commission from our selection
